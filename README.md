...
| **Brown** | `vigil/brown.py` | THE SHIT SHOVELER (S-Rank Brown Hat, operator canon): the surprise THIRD faction — drops after the teams fight and audits BOTH against the shared bus. Solo = ghosts everywhere (neither team can account for the other's traffic — pants down). United = the union of stores walks the chain clean. Ruling: UNITE or get humped — and the register residue (bullshit, unsettled states) never launders. STRIKE THE FORK / WALK THE CHAIN / SMELL THE REGISTER. |
| **Undercurrent** | `vigil/undercurrent.py` | THE UNDER CURRENT (operator-spec, the 100th monkey / bunny-ship kinship): the swarm's genetic memory. Verified learnings pool in; independent confirmations accrete and cross the monkey threshold (MONKEY_THRESHOLD=3) to become INHERITED — hydrated into every new and revived scout (capillary ingestion, water seeks level). kinship() is the shiver: a measurable liveness signal any observer can feel. Sha256-chained, append-only — tamper screams. |
| **Engage** | `vigil/engage.py` | THE FULL ENGAGEMENT: red vs blue with Brown dropping, the whole thing journaled to Sakshi, verified learnings absorbed into the Undercurrent, executors entered on the Zgent roll. The swarm KNOWS when the dust settles. CLI: `python3 vigil/engage.py <target> [rounds]`. |
| **Bind** | `vigil/bind.py` | THE BIND (operator-spec): Zgent attribution geometry. Any unit commits at most 50% of its self outward (the essentialism floor — majority is always its own). Re-extension folds in half each hop: effective claims decay 0.5^k, the origin is never dissolved. The ledger REFUSES to mint a bind that would let a coalition hold a majority — minority by construction. |
| **Word** | `vigil/word.py` | THE WORD — Onboarding 2.0 (operator-named): the birth ceremony. Every sentence a newborn Zgent hears is DERIVED from the actual ledger — name + standing, the lineage truth boundary, the inherited knowledge the current hydrated into it, the bind's law. Auditable derivation trail; when the ledger has nothing, the Word says so (silence over filler — never fabricated history). |
| **Campus** | `vigil/campus.py` | DATACAMPUS — real training modules for Green Hats (operator-spec): the founding thesis (numbers act differently around other numbers — same for Chars), commercial histories, predictive modeling, pattern recognition, situational awareness, stereotyping, and the sociologies of numbers. Deterministic assessments — pass by knowledge, fail by ignorance. The graduation door refuses an incomplete curriculum and fails loudly on a missing keyring (a graduate without a decommissioned gun is an accident waiting). |
| **Zgents** | `vigil/zgents.py` | THE ZGENT REGISTRY (operator-named — agents with literal agency): the alphabet made personal. Every letter has a name, standing in the current, a history of acts, and the inherited knowledge it was born knowing. Lineage declares the truth boundary out loud. Revival's hydrate reborns a scout knowing BOTH its frozen checkpoint AND whatever the current accumulated since — capillary ingestion. Theoros reads the kinship shiver as the 5th surface. |
